...
public class GetValuesActivity extends Activity implements OnGestureListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
				...
				.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
					public void onCheckedChanged(CompoundButton buttonView,
						...
						if (isChecked) {
							...
							// Button buttonTakeAgain = (Button)
							// findViewById(R.id.buttonTakeAgain);
							// buttonTakeAgain.setEnabled(false);
							...
							// Button buttonTakeAgain = (Button)
							// findViewById(R.id.buttonTakeAgain);
							// buttonTakeAgain.setEnabled(true);
						}
					}
				});
		...
		/* Novedades ComboBox */
		/*
		 * NewsListLoader.loadNews(); Spinner spinerNovedades = (Spinner)
		 * findViewById(R.id.spinnerNovedades); ArrayAdapter<New> adp1 = new
		 * ArrayAdapter<New>(this, android.R.layout.simple_list_item_1,
		 * NewsListLoader.getNews());
		 * adp1.setDropDownViewResource(android.R.layout
		 * .simple_spinner_dropdown_item); spinerNovedades.setAdapter(adp1);
		 * spinerNovedades.setOnItemSelectedListener(new
	...
	public void loadNextElementRec() {
		...
		if (this.autoNav.isChecked()) {
			if (!ElementListLoader.isFinRelevamiento()) {
				if (ElementListLoader.getPendingElements().get(elementIndex)
						.isRelevado()) {
					this.loadNextElement();
				} else {
	...
	public void loadNextElement() {
		if (this.autoNav.isChecked()) {
			boolean tomarMedicion = false;
			while (!ElementListLoader.isFinRelevamiento() && !tomarMedicion) {
				if (elementIndex < ElementListLoader.getPendingElements()
						.size() - 1) {
					elementIndex++;
				} else {
					elementIndex = 0;
			...
			if (ElementListLoader.isFinRelevamiento()) {
				...
				String msg = "Fin relevamiento, se han tomado todas las mediciones.";
	...
	private void showDebtAlert() {
		 ...
		 * 
		 * public void onTick(long millisUntilFinished) {toast.show();} public
		 * void onFinish() {toast.show();}
		 * 
		 * }.start();
		 */
	}
		...
		if (this.autoNav.isChecked()) {
			...
			while (!ElementListLoader.isFinRelevamiento() && !tomarMedicion) {
				if (elementIndex > 0) {
					...
					elementIndex = ElementListLoader.getPendingElements()
							.size();
				}
				if (!ElementListLoader.getPendingElements().get(elementIndex)
						.isRelevado()) {
	...
	/*
	 * private void saveNext99ElementsWithSameValue(Long actualValue) { for (int
	 * i = 0; i < 100; i++) { GetValuesActivity.this.loadNextElement();
	 * GetValuesActivity.this.getCurrentElement().setActualValue( actualValue);
	 * Store.saveElement(GetValuesActivity.this.getCurrentElement()); } }
	 */
	...
	private void loadNextElementDelayed() {
		this.elementActualValueEditText.setEnabled(false);
		// this.removeCallBacks();
		// this.autoNav.setChecked(false);
	...
	/*
	 * private boolean valorIgualAlAnterior() { return
	 * this.getCurrentElement().getActualValue() .equals(this.previousValue); }
	 */
